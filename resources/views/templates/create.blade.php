...
<div class="space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h2 class="text-2xl font-bold font-display dark:text-white">Créer un template</h2>
    </div>
    ...
    <form method="POST" action="{{ route('templates.store') }}" class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-[#161616] p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required
                       ...
                       placeholder="Ex: Lettre administrative standard">
            ...
            <div>
                ...
                <select name="type" required class="w-full rounded-xl border-gray-300 dark:border-white/[0.1] dark:bg-[#0C0C0C] dark:text-white focus:border-brand focus:ring-brand text-sm">
                    <option value="letter" {{ old('type') === 'letter' ? 'selected' : '' }}>Lettre</option>
                    <option value="invoice_proforma" {{ old('type') === 'invoice_proforma' ? 'selected' : '' }}>Facture Proforma</option>
                    <option value="invoice_final" {{ old('type') === 'invoice_final' ? 'selected' : '' }}>Facture Définitive</option>
                    <option value="contrat" {{ old('type') === 'contrat' ? 'selected' : '' }}>Contrat</option>
                    <option value="note_officielle" {{ old('type') === 'note_officielle' ? 'selected' : '' }}>Note Officielle</option>
                    <option value="page_garde" {{ old('type') === 'page_garde' ? 'selected' : '' }}>Page de Garde</option>
                </select>
            ...
            <div>
                ...
                <textarea name="content" rows="12" required
                          class="w-full rounded-xl border-gray-300 dark:border-white/[0.1] dark:bg-[#0C0C0C] dark:text-white focus:border-brand focus:ring-brand text-sm font-mono"
                          placeholder="Contenu du template...">{{ old('content') }}</textarea>
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">Définissez les variables sous forme JSON. Ex: <code class="text-brand">[{"key":"nom","label":"Nom","type":"text","required":true}]</code></p>
                <textarea name="variables" rows="5"
                          class="w-full rounded-xl border-gray-300 dark:border-white/[0.1] dark:bg-[#0C0C0C] dark:text-white focus:border-brand focus:ring-brand text-sm font-mono"
                          placeholder='[{"key":"nom_projet","label":"Nom du projet","type":"text","required":true,"placeholder":"Ex: Mon projet"}]'>{{ old('variables') }}</textarea>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-brand text-[#0C0C0C] text-sm font-semibold hover:bg-brand-hover transition">Créer</button>
        </div>
    </form>
</div>
@endsection
